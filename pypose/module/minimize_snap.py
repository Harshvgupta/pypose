...
class UAVTrajectoryPlanner(nn.Module):
    ...
    # def compute_directional_derivative(self, f, T, gi, h=0.001):
    #     # return (f(T + h * gi) - f(T)) / h
    #     return (f(T + h * gi) - f(T - h * gi)) / (2 * h)

    # def backtracking_line_search(self, f, T, gi, alpha=1.0, beta=0.5, improvement_threshold=0.01):
    #     cost_initial = f(T)
    #     print(gi)
    #     cost_new= f(T + alpha * gi)
    #     while cost_new > cost_initial - improvement_threshold * alpha:
    #         alpha *= beta
    #         cost_new = f(T + alpha * gi)
    #         # print("alpha",alpha)
    #         if alpha < 1e-3:
    ...
    #         # print(T_current)
    #         polys_x, polys_y = self.solve_minimum_snap1(waypoints[0], T_current, self.poly_order, self.start_vel[0], self.start_acc[0], self.end_vel[0], self.end_acc[0]), \
    #                            self.solve_minimum_snap1(waypoints[1], T_current, self.poly_order, self.start_vel[1], self.start_acc[1], self.end_vel[1], self.end_acc[1])
    #         return self.evaluate_cost_function(polys_x, polys_y,T_current)
    #     #  ----
    #     momentum = 0.8  # Momentum factor (this value can be tuned)
    #     velocity = torch.zeros_like(T)  # Initialize the velocity for momentum
    #     initial_learning_rate = 0.1  # Initial learning rate (this value can be tuned)
    #     decay_rate = 0.1
    #     #----
    #     gradients = torch.zeros_like(T)
    #     loss_values=[]
    #     for iteration in range(10):  # Adjust the number of iterations as needed
    #         # gradients = torch.zeros_like(T)  # Initialize gradients tensor to store gradients for each segment

    #         for i in range(1, len(T)):
    #             # Construct gi vector for directional derivative calculation
    #             # gi is designed to adjust segment i positively and all other segments negatively to keep the total time constant

    #             gi = torch.zeros_like(T)
    #             gi[i] = 1  # Set the i-th element to 1
    #             gi[gi == 0] = -1 / (len(T) - 1)  # Set all other elements to -1 / (m - 1)
    #             # Compute directional derivative
    #             grad = self.compute_directional_derivative(f, T, gi)
    #             print("grad is", grad)
    #             gradients[i] = grad  # Store the computed gradient

    #         # Normalize gradients to ensure their sum is 0, maintaining the total time constant
    #         # gradients -= gradients.mean()
    #         gradients_sum = gradients.sum()
    #         gradients -= gradients_sum / len(gradients)


    #         # Apply the learning rate decay based on the iteration
    #         # learning_rate = initial_learning_rate / (1 + decay_rate * iteration)
    #         learning_rate = initial_learning_rate * (momentum ** iteration)# Exponential decay

    #         # Update the velocity using the computed gradient and apply momentum
    #         velocity = momentum * velocity + learning_rate * gradients

    #         # Find the optimal step size using line search with the current velocity
    #         alpha = self.backtracking_line_search(f, T, velocity)

    #         # Update T using the computed optimal step size and the current velocity
    #         # T += alpha * velocity
    #         # # Ensure all time segments are non-negative
    #         # T = torch.clamp(T, min=0)
    #         # T_bnorm=T = torch.clamp(T, min=0)

    #         # # Normalize T to maintain total_time after adjustment
    #         # T = self.total_time * (T / T.sum())
    #         T_new = T + alpha * velocity
    #         if torch.any(torch.abs(T_new - T) > 1e-6):  # Check if there's a significant change
    #             T = T_new
    #         T = total_time * (T / T.sum())  # Normalize only after confirming changes are meaningful

    #         # print("hhhhhh")
    #         # print("Adjusted times:", T)
    #         print(f'Before normalization: {T_new}, After normalization: {T}')
    #         print(f'Iteration {iteration+1}, Learning Rate: {learning_rate}, Alpha: {alpha}')
    #         print(f'Updated Times: {T}')
    #         # Calculate the current loss value
    #         current_loss = f(T).item()
    ...
    # def evaluate_cost_function(self, polys_x, polys_y, time_stamps, lambda_reg=0.01):
    #     total_cost = 0
    #     num_segments = polys_x.shape[1]

    #     for segment in range(num_segments):
    #         segment_duration = time_stamps[segment + 1] - time_stamps[segment]

    #         # Get the snap coefficients for the current segment
    #         snap_x = polys_x[5, segment]
    #         snap_y = polys_y[5, segment]

    #         # Compute the cost for the current segment
    #         segment_cost = (snap_x**2 + snap_y**2) * segment_duration
    #         total_cost += segment_cost

    #     # Regularization terms (optional)
    #     # reg_cost_x = lambda_reg * torch.sum(polys_x**2)
    #     # reg_cost_y = lambda_reg * torch.sum(polys_y**2)
    #     # total_cost += reg_cost_x + reg_cost_y
